 ...
 * @internal
 */
class Prefix extends \CharlotteDunois\Livia\Commands\Command {
    ...
    function run(\CharlotteDunois\Livia\CommandMessage $message, array $args, bool $fromPattern) {
        return (new \React\Promise\Promise(function (callable $resolve, callable $reject) use ($message, $args) {
            ...
            if($message->message->guild !== null) {
                if($message->message->member->hasPermission('ADMINISTRATOR') === false && $this->client->isOwner($message->message->author) === false) {
